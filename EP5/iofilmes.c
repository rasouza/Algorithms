...
mostreMelhoresFilmes(ListaFilmes *lst)
{
	...
	printf("\n\nQual o numero de filmes a serem mostrado: ");
	scanf("%d", &filmes);

	printf("\n\nQual a nota maxima: ");
	scanf("%f", &nota);

	printf("\n\nQual o numero minimo de votos de cada filme: ");
	scanf("%d", &votos);

	for(n = 0, lista=lst->cab->ant; n < filmes || lista != lst->cab; lista = lista->ant)
		if(lista->votos >= votos && lista->nota <= nota)
		{
			mostreFilme(lista);
			n++;
		}
...
mostrePioresFilmes(ListaFilmes *lst)
{
	...
	printf("\n\nQual o numero de filmes: ");
	scanf("%d", &filmes);

	printf("\n\nQual a nota maxima dos filmes: ");
	scanf("%f", &nota);
